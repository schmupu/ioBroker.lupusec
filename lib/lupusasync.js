 ...
 */
  ...
  async getWebcamSnapshots() {
    ...
    for (let i = 1; i <= 8; i++) {
      ...
      if (this.webcams[i] && this.webcams[i].url === url && this.webcams[i].name === name) {
        continue;
      }
      this.webcams[i] = {
        name: name,
      ...
      if (url && url.length > 0) {
        ...
        let cam = new webcam.Webcam(this.adapter, opt);
        stateid = channelid + '.stream';
        let portstream = port + (i - 1);
        cam.listenServerStream(host, portstream);
        promises.push(this.setObjectOrUpdate(stateid, {
          type: 'state',
          common: {
            type: 'string',
          ...
          native: {}
        }, 'http://' + host + ':' + portstream, true));
        stateid = channelid + '.image';
        let portimage = port + 8 + (i - 1);
        cam.listenServerImage(host, portimage);
        promises.push(this.setObjectOrUpdate(stateid, {
          type: 'state',
          common: {
            type: 'string',
          ...
          native: {}
        }, 'http://' + host + ':' + portimage, true));
        cam.writeImageToCallback(async (stream) => {
          await this.adapter.writeFileAsync(this.adapter.namespace, '/image/cam' + i + '.jpg', stream);
        });
        cam.writeStreamToCallback(async (stream) => {
          await this.adapter.writeFileAsync(this.adapter.namespace, '/stream/cam' + i + '.mjpg', stream);
        });
        cam.start();
      }
    }
