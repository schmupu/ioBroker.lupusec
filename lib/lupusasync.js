  ...
  async getWebcamSnapshots() {
    ...
    for (let i = 1; i <= 8; i++) {
      ...
      if (url && url.length > 0) {
        ...
        cam.listenServerStream(portstream);
        promises.push(this.setObjectOrUpdate(stateid, {
          type: 'state',
          common: {
            type: 'string',
          ...
          native: {}
        }, host + ':' + portstream, true));
        stateid = channelid + '.image';
        let portimage = port + 8 + (i - 1);
        cam.listenServerImage(portimage);
        promises.push(this.setObjectOrUpdate(stateid, {
          type: 'state',
          common: {
            type: 'string',
        ...
        cam.writeImageToCallback(async (stream) => {
          await this.adapter.writeFileAsync(this.adapter.namespace, 'test.jpg', stream);
        });
        cam.writeStreamToCallback(async (stream) => {
          await this.adapter.writeFileAsync(this.adapter.namespace, 'test.mjpg', stream);
        });
        cam.start();
      }
    }
