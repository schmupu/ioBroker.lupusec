  ...
  */
  ...
  async deviceListDevicePSSListSetStatus(object) {
    ...
    for (let devicename in object) {
      ...
      this.areazones[area + '#' + zone] = {
        sid: device.sid || device.id,
        area: device.area,
        zone: device.zone,
        type: device.type
      };
  ...
  async logsGetPost() {
    ...
    for (let l in logrows) {
      ...
      const regex2 = /{WEB_MSG_DEVICE_AREA_ZONE}[\t\s]*(\d*)[\t\s]*(\d*)/gm;
      let m2 = regex2.exec(logrow.source);
      if (m2 && m2[1] && m2[2]) {
        try {
          // alle Objekte lesen
          let actualstates = await this.adapter.getStatesAsync('*');
          const sid = Object.keys(actualstates).find((key) => {
            return (key.endsWith('zone') && actualstates[key].val == m2[2] && actualstates[key.replace(/zone$/, 'area')].val == m2[1]);
