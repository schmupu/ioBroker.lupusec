...
// *******************************************************************************
Lupus.prototype.setObjectStateOld = function(id, status) {
    ...
    for (let devicename in object) {
      ...
      if (Object.prototype.toString.call(device) === '[object Object]') {
        ...
        for (let statusname in device) {
          ...
          device.type_f = self.getTypeList(device.type) || device.type_f;

          // Stecker hat Schreibberechtigung
          if ((device.type == 48 && statusname == "status_ex") || // switch
            (device.type == 74 && statusname == "hue") || // HUE
            (device.type == 74 && statusname == "sat") || // HUE
            (device.type == 74 && statusname == "level") || // HUE
            (device.type == 66 && statusname == "level")) // Dimmer
          {
            write = true;
          } else {
            write = false;
          ...
          if (type !== null) {
            ...
            let parameter = self.getDatapointDeviceList(device.type, statusname);

            if (parameter) {
              self.adapter.log.info("hhh");
            }
            ...
            // self.setObjectState(id, status, statusname, type, write, tms);
            self.setObjectState(id, {
              val: status,
              name: 'State ' + device.name + ' ' + statusname,
              type: type,
              write: write,
              ts: tms
            });
      ...
      if (Object.prototype.toString.call(device) === '[object Object]') {
        ...
        for (let statusname in device) {
          ...
          if (parameter) {
            ...
            if (parameter.name) {
              parameter.name = 'State ' + device.name + ' ' + parameter.name;
            } else {
              parameter.name = 'State ' + device.name + ' ' + statusname;
            }
            ...
            if (statusname == "alarm_status") {
              ...
              statusname = statusname + "_ex";
              parameter = self.getDatapointDeviceList(device.type, statusname);
              if (parameter) {
      ...
      if (Object.prototype.toString.call(device) === '[object Object]') {
        ...
        for (let statusname in device) {
          ...
          if ((device.type == 48 && statusname == "status_ex") || // switch
            (device.type == 74 && statusname == "hue") || // HUE
            (device.type == 74 && statusname == "sat") || // HUE
            (device.type == 74 && statusname == "level") || // 48
            (device.type == 66 && statusname == "level")) // Dimmer
          {
            write = true;
          } else {
