...
class Lupus {
  ...
  */
  ...
  async deviceGenericPost(url, form, name) {
    try {
      ...
      if (body) {
        // body = this.delSonderzeichen(body);
        // body = JSON.parse(body);
        body = await this.JSONparse(body, name);
  ...
  async deviceGenericGet(url, form, name) {
    try {
      if (!await this.isPollingAlloed()) return;
      let opt = {
        url: this.getURL(url),
  ...
  async systemGet(form) {
    let body = await this.deviceGenericGet(urlSystemGet, form, urlSystemGet);
    let sysform;
