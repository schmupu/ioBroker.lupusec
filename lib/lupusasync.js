      ...
      for (let statusname in device) {
        ...
        if (statusname === 'alarm_status') {
          if (this.adapter.config.alarm_status) {
            ...
            // Alarm wird ausgelöst, der aktuelle Status des Sensors wird gespeichert  
            if (statusAlarm > 0 && statusvalue) {
              this.setState(statusid + '_old', statusvalue);
            }
            let statusvalueOld = this.getState(statusid + '_old');
            // Wenn Alarm ausgelöst wird oder Modus ist ARM, HOME1/2/3 u. der Sensorsatus ist leer
            if ((statusAlarm > 0 || mode > 0) && !statusvalue && statusvalueOld) {
              statusvalue = statusvalueOld;
            }
            this.setState(statusid + '_old', statusvalue);
          }
