...
// Id und Types miteinander verknüpfen
// *******************************************************************************
Lupus.prototype.getExtendObjectById = function(id) {

  if (this.updateobject[id] === true) {
    return true;
  } else {
    this.updateobject[id] = true;
    return false;
  }
...
// *******************************************************************************
Lupus.prototype.setAllStateChange = function() {
...
Lupus.prototype.propertiesObjAinObjB = function(obja, objb) {
  ...
  /*
  // check if properties of object b is in object a
  for (let p in objb) {
    if (objb.hasOwnProperty(p) && !obja.hasOwnProperty(p)) return false;
  }
  */
...
Lupus.prototype.setObjectOrUpdate = function(id, parameter, callback) {
  ...
  self.adapter.getObject(id, function(err, obj) {
    ...
    if (!err && obj) {
      ...
      if (parameter.common.delete && parameter.common.delete == true) {
        ...
        // Nur einmal beim Start pro ID durchlaufen
        if (self.getExtendObjectById(id) === false && !self.propertiesObjAinObjB(parameter.common, obj.common)) {

          self.adapter.extendObject(id, parameter, function() {
            return callback && callback();
          });
      ...
      // Nur Objekt anlegen, wenn delete == false oder delete parameter
      // fehlt
      if (!parameter.common.delete || parameter.common.delete == false) {
...
Lupus.prototype.setStateForDevice = function(id, statusname, statusvalue, devicename, devicetype, ts) {
  ...
  if (parameter) {
    ...
    // if parameter set, show all sates (not reccomended)
    parameter = {
      val: statusvalue,
      name: devicename + ' ' + statusname,
      type: type,
      role: 'value',
      read: true,
      write: false,
        ...
        */
        ...
        if (statusname == "status" && device.type == 54) {
          ...
          if (m) {
            ...
            tmpid = channelid + '.' + tmpname;
            // self.setStateForDevice(tmpid, tmpname, actualhumidity, device.name, device.type, ts);
            newstates[tmpname] = {
              statusid: tmpid, // device.RFxxxxx.status
              statusname: tmpname, // status
