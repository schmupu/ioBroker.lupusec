  ...
  function setStatus(object) {
    ...
    for (let devicename in object) {
      ...
      if (Object.prototype.toString.call(device) === '[object Object]') {
        ...
        self.createObjectForDevice(device.sid + '.info', device.name, device.type);
        ...
        for (let statusname in device) {
          ...
          self.setStateForDevice(id, statusname, status, device.name, device.type, tms);
  ...
  function setStatus(object) {
    ...
    for (let devicename in object) {
      ...
      if (Object.prototype.toString.call(device) === '[object Object]') {
        ...
        self.createObjectForDevice(device.id + '.info', device.name, device.type);
...
Lupus.prototype.PanelCondGet = function() {
  ...
  function setStatus(object) {
    ...
    self.createObjectForStatus(sid, devicename);
    ...
    for (let statusname in objectnew) {
      ...
      self.setStateForStatus(id, statusname, status, tms);
