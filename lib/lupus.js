...
// *******************************************************************************
Lupus.prototype.getAppleStautusFromLupusec = function (mode_pc_a, alarm_ex) {
...
// *******************************************************************************
Lupus.prototype.getLupusecFromAppleStautus = function (applestatus) {
  ...
  for (let id in this.statechange) {
    ...
    if (type) {
      ...
      if (!parameter || Object.keys(parameter).length == 0) {
        self.adapter.delObject(id, function () {
...
Lupus.prototype.getTypeList = function (lupusType) {
  ...
  ret = types.typeList['TYPE_' + lupusType];
...
Lupus.prototype.getDatapointDeviceList = function (lupusType, name) {
  ...
  if (ret) {
    ...
    dp = types.dpDeviceList[ret.devlist][name] || types.dpDeviceList.type_all[name] || null;
...
Lupus.prototype.createObjectForDevice = function (deviceid, devicename, devicetype) {
  ...
  let ret = this.getTypeList(devicetype);
  let a = types.dpDeviceList.type_all || {};
  let b = {};
  let icon = '';

  if (ret) {

    b = types.dpDeviceList[ret.devlist] || {};
    icon = ret.icon || '';
  ...
  if (status.hasOwnProperty('val')) {
    ...
    self.adapter.getState(id, function (err, state) {
      ...
      if (!err) {
        ...
        // if (state === null || (state.val != status.val && state.ack === true) || (state.ack === false && status.ts > (state.ts + (self.getPollSec() / 2)))) {
        if (state === null || (state.val != status.val && state.ack === true) || state.ack === false) {
    ...
    request.get(options, function (error, response, body) {
      ...
      if (error || response.statusCode !== 200) {
        ...
        try {
          ...
          self.adapter.log.debug('TokenGet (JSON.parse) : ' + JSON.stringify(body));
...
Lupus.prototype.PanelCondGet = function () {
  ...
  function setStatus(object) {
    ...
    let apple_home_a2;
    let statusid;
    apple_home_a1 = self.getAppleStautusFromLupusec(objectnew.mode_pc_a1, objectnew.alarm_ex);
    apple_home_a2 = self.getAppleStautusFromLupusec(objectnew.mode_pc_a2, objectnew.alarm_ex);
    statusid = deviceid + '.apple_home_a1';
    self.setStateForStatus(statusid, 'apple_home_a1', apple_home_a1, ts);
    statusid = deviceid + '.apple_home_a2';
    self.setStateForStatus(statusid, 'apple_home_a2', apple_home_a2, ts);
