 ...
 */
  ...
  async setPollSecOk() {
    if (this.errorWaitSec > 0) {
      this.process.setPollSec(this.adapter.config.alarm_polltime);
      this.adapter.log.info('Poll rythmus chnaged back after an Error from ' + this.errorWaitSec + ' to ' + this.process.getPollSec() + ' seconds.');
      this.errorWaitSec = 0;
    }
    this.process.setError(false);
  }
  ...
  async setPollSecError(message) {
    if (!this.process.getError()) {
      this.process.setError(true);
      if (!this.errorWaitSec) this.errorWaitSec = 0;
      this.errorWaitSec = this.errorWaitSec < 300 ? this.errorWaitSec += 15 : 300;
      this.process.setPollSec(this.errorWaitSec);
  ...
  async getToken(newtoken) {
    // try {
    let timeout = 1 * 60; // 15 * 60; // to get every 15 minutes a new token 
    if (!this.token || !this.token.token || newtoken || Math.abs(getNow() - this.token.time) > timeout) {
      let token;
      let opt = {
  ...
  async deviceListDevicePSSListSetStatus(object) {
    ...
    for (let devicename in object) {
      ...
      for (let statusname in device) {
        ...
        // Nuki
        if (statusname === 'status' && device.type === 57) {
          let reg = /\{WEB_MSG_DL_(.+)\}(.*)/gm;
          let m = reg.exec(statusvalue);
          if (m) {
            statusvalue = m[1] ? m[1].trim() : statusvalue;
          }
        }
        ...
        if (statusname === 'nuki' && device.type === 57) {
          ...
          tmpname = 'nuki_action';
          tmpid = channelid + '.' + tmpname;
          let oldstatus = this.getState(statusid);
