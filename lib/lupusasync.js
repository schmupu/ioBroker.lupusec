...
class Lupus {
  ...
  setPollSec(pollsec) {
    ...
    // return this.process.setPollSec(pollsec);
  }
 ...
 */
    ...
    return new Promise(async (resolve) => {
      try {
        if (id && obj) {
          ...
          if (obj && objold && obj.common && objold.common && obj.common.name !== objold.common.name) {
          ...
          if (typeof value !== 'undefined') {
            ...
            let difftime = unixtime - (oldstate.ts + this.getPollSec() * 1000) || 0;
  ...
  /**
     ...
     */
  async addToProcessNew(funct, prio, loop) {
    ...
    if (process) {
      if (process.funct) {
        ...
        try {
          ...
          // this.adapter.log.debug('Process result: ' + JSON.stringify(result));
        } catch (error) {
