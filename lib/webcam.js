...
class Webcam {
  ...
  serverListen(port, host) {
    if (port > 0 && host) {
      this.adapter.log.info('Starting Webcam Listining Service');
  ...
  checkClients() {
    setInterval(() => {
      for (let i in this.requests) {
        if (this.requests[i].req && !this.requests[i].callback) {
          let found = false;
          let reqid = i;
  ...
  startRequest(opt, id, callback) {
    ...
    req.on('end', () => {
      ...
      if (this.requests[id]) this.requests[id].req = undefined;
