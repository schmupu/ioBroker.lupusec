  ...
  checkClients() {
    setInterval(() => {
      ...
      for (let i in this.requests) {
        if (this.requests[i].req && !this.requests[i].callback && (this.requests[i].ts + this.wait) <= unixtime) {
          let found = false;
          let reqid = i;
          for (let j in this.client) {
            let url = this.client[j].url;
          ...
          converter.mjpegTojpeg(chunk, (img) => {
            ...
            for (let i = this.client.length; i--;) {
              if (this.client[i].url === '/image/' + id) {
                this.requests[id].ts = unixtime;
                this.client[i].res.end(img);
              }
            }
            // close request if no client is streaming or requesting image
            if (!this.requests[id].callback && (this.requests[id].ts + this.wait) <= unixtime) {
