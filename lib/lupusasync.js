 ...
 */
  ...
  async getWebcamSnapshots() {
    ...
    let host = this.adapter.host;
    let stateurl = '';
    // let channels = await this.adapter.getChannelsOfAsync(deviceid);
    if (port === 0) {
      return;
    }
    if (!this.webcam) this.webcam = new webcam.Webcam(this.adapter, port, '0.0.0.0');
    await this.setObjectOrUpdate(deviceid, {
      type: 'device',
      common: {
        name: devicename,
    ...
    for (let i = 1; i <= 8; i++) {
      ...
      if (url.length > 0) {
        ...
        this.webcam.startRequestStream(opt, reqstreamname);
      }
      stateid = channelid + '.stream';
      stateurl = url.length > 0 ? 'http://' + host + ':' + port + reqstreamname : '';
      promises.push(this.setObjectOrUpdate(stateid, {
        type: 'state',
        common: {
          type: 'string',
      ...
      stateurl = url.length > 0 ? 'http://' + host + ':' + port + reqimagename : '';
      promises.push(this.setObjectOrUpdate(stateid, {
        type: 'state',
        common: {
          type: 'string',
