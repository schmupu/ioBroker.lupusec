...
class Lupus {
  ...
  setPollSecOk() {
    if (this.pollsec > this.adapter.config.alarm_polltime) {
      this.adapter.log.info('Poll rythmus chnaged back to ' + this.adapter.config.alarm_polltime + ' seconds');
    }
    this.pollsec = this.adapter.config.alarm_polltime;
  }
  ...
  async addToProcess(funct, prio, loop) {
    ...
    for (let i = 0; i < this.p.length; i++) {
      if (this.p[i].prio > prio) {
        let tmp = [];
        this.p = tmp.concat(this.p.slice(0, i), {
          prio: prio,
          funct: funct,
          loop: loop
        }, this.p.slice(i));
        // if prio 1, start processes immediately
        if (prio == 1 && this.pst && !this.error) {
   ...
   */
  async startProcess() {
    if (this.p) this.adapter.log.debug('Prcoess Queue: ' + JSON.stringify(this.p));
    let f = this.p.shift();
    if (f) {
      if (f.funct) {
        this.adapter.log.debug('Prcoess: ' + JSON.stringify(f));
        // clear running timeouts
        this.pst && clearTimeout(this.pst);
        this.pst = undefined;
        try {
          let result = isAsync(f.funct) ? await f.funct() : f.funct();
        } catch (error) {
          this.adapter.log.error('could not procces functions in startProcess: ' + error);
        }
      }
      if (f.loop) {
        this.addToProcess(f.funct, f.prio, f.loop);
      }
    }
    this.pst = setTimeout(() => {
      this.startProcess();
   ...
   */
  async getToken() {
    this.adapter.log.debug('Starting Token Request in PostTokenRequest');
    let opt = {
    ...
    try {
      ...
      if (body) {
        ...
        token = body.message;
        this.adapter.log.debug('TokenGet (JSON.parse) : ' + JSON.stringify(body));
      }
      this.setPollSecOk();
      return token;
    } catch (error) {
      this.adapter.log.info('TokenGet : ' + error); // error in the above string (in this case, yes)!
      this.setPollSecError();
    }
  }

  async postRequest(options) {
    let opt = copyObject(options);
    ...
    try {
      ...
      this.adapter.log.error('Fehler in Post Request ' + error);
      this.setPollSecError();
    }
  }
  ...
  async deviceGenericPost(url, form) {
    ...
    return await this.postRequest(opt);
  }
  ...
  async deviceGenericGet(url, form) {
    ...
    try {
      ...
      if (body) {
        body = JSON.parse(body);
        this.adapter.log.debug('deviceGenericGet (JSON.parse) : ' + JSON.stringify(body));
      }
      this.setPollSecOk();
      return body;
    } catch (error) {
      this.adapter.log.info('deviceGenericGet : ' + error); // error in the above string (in this case, yes)!
      this.setPollSecError();
    }
  }
  ...
  async deviceEditThermoGet(form) {
    let body = await this.deviceGenericGet(urlDeviceEditThermoGet, form);
    return body;
  }
