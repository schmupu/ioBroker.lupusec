...
class Lupus {
  ...
  async JSONparse(data, id) {
    let result;
    if (id && this.jsonparse[id] && this.jsonparse[id].data === data)
      return {}; // return this.jsonparse[id].result;
    if (this.adapter.config.alarm_cpu === false) {
      result = await JSON.parse(delSonderzeichen(data));
    } else {
      result = await (await SingletonHelper.getInstance()).JSONparse(data);
    }
    if (id) {
      this.jsonparse[id] = {
        data: data,
  ...
  */
  ...
  async getToken(newtoken) {
    ...
    if (!this.token || !this.token.token || newtoken || Math.abs(getNow() - this.token.time) > this.tokentimeout) {
      ...
      let body = await (await SingletonHelper.getInstance()).requestGet(opt);
  ...
  async deviceGenericPost(url, form) {
    try {
      ...
      let body = await (await SingletonHelper.getInstance()).requestPost(opt);
      // let body = await request.post(opt);
      if (body) {
        // body = this.delSonderzeichen(body);
        // body = JSON.parse(body);
        body = await this.JSONparse(body, url + (form && form.id ? '_' + form.id : ''));
