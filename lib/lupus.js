...
Lupus.prototype.logging = function(text, deb = false) {
  ...
  if (deb === true) {
    ...
    if (debug === true) {
      ...
      this.adapter.log.debug("Debug: " + text);
...
Lupus.prototype.setObjectState = function(id, state) {

  var self = this;
  var status = state.val;
  var statusname = state.name;
  var type = state.type;
  var write = state.write;
  var tms = state.ts;
    ...
    if (!err && obj) {
      ...
      self.adapter.getState(id, function(err, state) {
        ...
        if (!err) {
          ...
          if (state === null ||
            (state.val != status && state.ack === true) ||
            (state.ack === false && tms > (state.ts + (self.pollsec / 3)))) {
...
Lupus.prototype.LogoutPost = function() {
  ...
  function parse(error, body) {
    ...
    if (error || ret != 1) {
      ...
      self.logging("Fehler LogoutPost");
...
Lupus.prototype.PanelCondPost = function(area, mode) {
  ...
  function parse(error, body) {
    ...
    if (error || ret != 1) {
      ...
      self.logging("Fehler PanelCondPost");
...
Lupus.prototype.DeviceSwitchPSSPost = function(id, parameter) {
  ...
  function parse(error, body) {
    ...
    if (error || ret != 1) {
      ...
      self.logging("Fehler DeviceSwitchPSSPost");
...
Lupus.prototype.DeviceEditPost = function(id, parameter) {
  ...
  function parse(error, body) {
    ...
    if (error || ret != 1) {
      ...
      self.logging("Fehler DeviceEditPost");
