 ...
 */
  ...
  async getWebcamSnapshots() {
    ...
    if (!this.adapter.config.webcam_providing) {
      this.adapter.deleteDevice(deviceid);
      return;
    }
    ...
    for (let i = 1; i <= 8; i++) {
      ...
      if (port > 0) {
        ...
        if (url.length > 0) {
          ...
          this.webcam.startRequest(opt, reqid);
          /*
          this.webcam.startRequest(opt, reqid, async (image, stream) => {
            if (port !== 0) {
              if (image) {
                let path = './video/';
                if (!fs.existsSync(path)) {
                  ...
                  fs.writeFileSync(path + reqid + '.jpg', image);
                }
                await this.adapter.writeFileAsync(this.adapter.namespace, reqimagename + '.jpg', image);
              }
            }
            if (stream) {
              // if stream do something else
            }
          });
          */
        }
