...
// *******************************************************************************
Lupus.prototype.getTypeList = function(type) {


  let ret = null;

  ret = typeList['D_TYPE_' + type];
  return ret;
...
Lupus.prototype.getDatapointDeviceList = function(lupusType, name) {
  ...
  dp = dpDeviceList['type_' + lupusType][name] || dpDeviceList.default.lupusType[name];
  return dp;
...
Lupus.prototype.DeviceListGet = function() {
  ...
  function read() {
    ...
    self.GetRequest(options, function(error, data) {
      ...
      timeout = setTimeout(read, self.pollsec * 1000);
...
Lupus.prototype.DeviceEditGet = function(id) {
  ...
  function read() {
    ...
    self.PostRequest(options, function(error, data) {
      ...
      parse(error, data);
      // clearTimeout(timeout);
      timeout = setTimeout(read, 4 * self.pollsec * 1000);
