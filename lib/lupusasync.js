...
class Lupus {
      ...
      try {
        if (id && obj) {
          ...
          if (typeof value !== 'undefined') {
            ...
            if (!oldstate || oldstate.val != value || oldstate.ack !== ack || this.getState(id) != value || renew) {
              if (this.adapter.config.alarm_equal && !oldstate) {
                await this.adapter.setStateAsync(id, value, ack);
                this.setState(id, value);
              } else if (this.adapter.config.alarm_equal && oldstate.val == value && oldstate.ack === false) {
                // ack gesetzt, sonst ist alles so geblieben
                await this.adapter.setStateAsync(id, value, ack);
                this.setState(id, value);
              } else if (this.adapter.config.alarm_equal && (difftime >= 0 || oldstate.ack === ack)) {
                await this.adapter.setStateAsync(id, value, ack);
                this.setState(id, value);
              } else if (!this.adapter.config.alarm_equal) {
                await this.adapter.setStateAsync(id, value, ack);
                this.setState(id, value);
              }
            }
  ...
  */
  ...
  async logsGetPost() {
    ...
    body = await this.deviceGenericPost(urlrecordListGet, form);
    if (body && body.logrows && !isEmpty(body.logrows)) recordrows = body.logrows;
    body = await this.deviceGenericPost(urlLogsGet, form);
    if (body && body.logrows && !isEmpty(body.logrows)) logrows = body.logrows;
    if (logrows && recordrows) {
      for (let r in recordrows) {
        let sid = recordrows[r].sid;
        if (this.logmsg[sid] && this.logmsg[sid].time >= recordrows[r].time) continue;
        let logrow = logrows.find((v) => { return v.log_time === recordrows[r].time; });
        // get device type
        const regex = /{D_TYPE_(\d+)}/gm;
        let m = regex.exec(recordrows[r].type_f);
        if (logrow && m) {
          this.logmsg[sid] = {
            sid: recordrows[r].sid,
            time: recordrows[r].time,
            area: recordrows[r].area,
            zone: recordrows[r].zone,
            event: recordrows[r].event,
            name: recordrows[r].name,
            type_f: recordrows[r].type_f,
            type: m[1],
            action: logrow.action,
            logmsg: logrow.msg
          };
          // Something changed, we will update 
          let object = this.logmsg[sid];
