  ...
  for (let devicename in object) {
    ...
    if (Object.prototype.toString.call(device) === '[object Object]') {
      ...
      for (let statusname in device) {
        ...
        // "{WEB_MSG_DC_CLOSE} or "{WEB_MSG_DC_CLOSE}
        ...
        if (statusname == "status" && device.type == 48) {
          ...
          const regpower = /\{WEB_MSG_PSM_POWER\}(.+)\{WEB_MSG_POWER_METER_ENERGY\}(.+)/gm;
          let m = regpower.exec(statusvalue);
          let power = 0;
          let powertotal = 0;
          let tmpname;
          let tmpid;
  ...
  function setStatus(object) {
    ...
    for (let devicename in object) {
      ...
      if (Object.prototype.toString.call(device) === '[object Object]') {
        ...
        let deviceid = 'devices';
        let channelid = deviceid + '.' + device.sid;
  ...
  function setStatus(object) {
    ...
    for (let devicename in object) {
      ...
      if (Object.prototype.toString.call(device) === '[object Object]') {
        ...
        let deviceid = 'devices';
        let channelid = deviceid + '.' + device.id;
