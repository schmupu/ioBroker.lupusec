 ...
 */
  ...
  async addToProcess(funct, object) {
    return this.process.addToProcess(funct, object);
  }
  ...
  async delFromProcess(key) {
    return this.process.delFromProcess(key);
  }
  ...
  async getWebcamSnapshots() {
    ...
    // If webcam not provided, set port to 0
    if (!this.adapter.config.webcam_providing) port = 0;
    // let channels = await this.adapter.getChannelsOfAsync(deviceid);
    // if (!this.webcam && port > 0) this.webcam = new webcam.Webcam(this.adapter, port, bind);
    ...
    for (let i = 1; i <= 8; i++) {
      ...
      let reqimagename = '/image/cam' + i + '.jpg';
      let reqstreamname = '/stream/cam' + i + '.mjpg';
      let stateurl = '';
      if (port !== 0) {
        if (this.webcams[i] && this.webcams[i].url === url && this.webcams[i].name === name && this.webcam.isRequestExist(reqimagename) && this.webcam.isRequestExist(reqstreamname)) {
          continue;
        }
        this.webcams[i] = {
          name: name,
        ...
        if (url.length > 0) {
          ...
          this.webcam.closeRequest(reqimagename);
          this.webcam.startRequestImage(opt, reqimagename, async (image) => {
            if (port !== 0) {
              if (!fs.existsSync('./video')) {
                fs.mkdirSync('./video');
              } else {
                fs.writeFileSync('./video/cam' + i + '.jpg', image);
              }
              await this.adapter.writeFileAsync(this.adapter.namespace, reqimagename, image);
            }
          });
          this.webcam.closeRequest(reqstreamname);
          this.webcam.startRequestStream(opt, reqstreamname, async (stream) => {
            if (port !== 0) {
              // await this.adapter.writeFileAsync(this.adapter.namespace, reqstreamname, stream);
            }
          });
        }
      } 
