...
async function JSONparse(data) {
  data = await ( await JSONtools.getInstance()).JSONparse(data);
...
class JSONtools {
  ...
  constructor() {
    JSONtools.instance=this;
  }

  static async getInstance() {
    if (!JSONtools.instance) {
      JSONtools.instance=new JSONtools();
    }
    return JSONtools.instance;
  }
...
class Lupus {
  ...
  */
  ...
  async promiseAll(promises) {
    // this.adapter.config.alarm_cpu = false;
    if (promises && promises.length > 0) {
      if (this.adapter.config.alarm_cpu === false) {
  ...
  */
  ...
  async getToken(newtoken) {
    ...
    if (!this.token || !this.token.token || newtoken || Math.abs(getNow() - this.token.time) > this.tokentimeout) {
      ...
      this.adapter.log.debug('Form of getToken (' + urlTokenGet + ') : ' + JSONstringify(opt));
      let body = await ( await JSONtools.getInstance()).requestGet(opt);
      // let body = await request.get(opt);
      if (body) {
        // body = this.delSonderzeichen(body);
        // body = JSON.parse(body);
        body = await JSONparse(body);
        token = body.message;
        this.adapter.log.debug('TokenGet : ' + JSONstringify(body));
      }
  ...
  async deviceGenericPost(url, form) {
    try {
      ...
      this.adapter.log.debug('Form of deviceGenericPost (' + url + ') : ' + JSONstringify(opt));
      let body = await ( await JSONtools.getInstance()).requestPost(opt);
      // let body = await request.post(opt);
      if (body) {
        // body = this.delSonderzeichen(body);
        // body = JSON.parse(body);
        body = await JSONparse(body);
        this.adapter.log.debug('Result of deviceGenericPost (' + url + ') : ' + JSONstringify(body));
      }
      await this.setPollSecOk();
      if (body.result === 0) {
        switch (body.message) {
  ...
  async test(url, form, method) {
    try {
      let opt = {
        url: 'https://alarm.stueben.local/' + url,
        responseType: 'json',
        method: 'POST',
        rejectUnauthorized: false,
        timeout: 30 * 1000,
        ...
        transformResponse: (data) => JSON.parse(delSonderzeichen(data)),
      };
      if (form) opt.form = form;
  ...
  async deviceGenericGet(url, form) {
    try {
      ...
      if (form) opt.form = form;
      this.adapter.log.debug('Form of deviceGenericGet (' + url + ') : ' + JSONstringify(opt));
      // let body = await this.test(url, form, 'GET');
      // let body = await request.get(opt);
      let body = await ( await JSONtools.getInstance()).requestGet(opt);
      if (body) {
        // body = this.delSonderzeichen(body);
        // body = JSON.parse(body);
        body = await JSONparse(body);
        this.adapter.log.debug('Result of deviceGenericGet (' + url + ') : ' + JSONstringify(body));
      }
      await this.setPollSecOk();
      if (body.result === 0) {
        switch (body.message) {
    ...
    if (object && Object.prototype.toString.call(object) === '[object Object]') {
      ...
      for (let statusname in newstates) {
        ...
        if (parameter) {
          ...
          if (this.adapter.config.alarm_cpu && parameter.hasOwnProperty('polltime')) {
            let unixtime = Math.round(new Date().getTime());
  ...
  isEqualGetPostRequest(id, obj) {
    ...
    let objString = this.hashcode(JSONstringify(obj)); // JSONstringify(obj);
